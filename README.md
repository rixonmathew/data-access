...
| [**`ducklake`**](ducklake/README.md) | SQL-Catalog Lakehouse | `postgres:17` & `localstack:3.4.0` (S3) | DuckLake open table format on embedded DuckDB 1.5 with a PostgreSQL metadata catalog (`ATTACH 'ducklake:postgres:...'`), partitioned tables (`SET PARTITIONED BY (year(sale_date), region)`), data inlining and flush to per-partition Parquet files, catalog metadata verified directly in PostgreSQL, Hive-partitioned Parquet on local disk and S3 with partition pruning, REST query API. | ✅ **100% Green** |
